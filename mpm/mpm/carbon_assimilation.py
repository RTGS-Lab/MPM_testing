...
class CarbonAssimilation:
    ...
    def compute_Ci(self, Leaf_Temp, VPD):
        ...
        Calculations are derived from Leuning (1995) which present a model of how stomatal conductance
    ...
    def photosynthesis(self, Leaf_Temp, Absorbed_PAR, VPD):
        """Calculate the net photosynthesis rate of a C3 plant canopy.
        
        From reference: Farquhar G.D., von Caemmerer S. & Berry J.A. (1980) A biochemical model of
        photosynthetic CO2 assimilation in leaves of C3 species. Planta 149, 78–90.
        
        Parameters:
            Absorbed_PAR (float): Photosynthetically Active Radiation absorbed by the canopy [W m⁻²]
            Leaf_Temp (float): Canopy temperature [°C]
            VPD (float): Vapor pressure deficit [kPa]
            
        Returns:
            float: Photosynthesis rate [µmol CO₂ m⁻² s⁻¹] (leaf area basis)
        """
        # constants
    ...
    def sunlit_shaded_photosynthesis(self, environmental_variables):
        ...
        Args:
            environmental_variables (dict): Dictionary of environmental variables
            
        Returns:
            tuple: (photosynthesis_sunlit, photosynthesis_shaded) in µmol CO₂ m⁻² s⁻¹
        """
